...
st.write(
    ...
    **How it works:**
    - Shelves can become empty randomly (simulate customer purchases).  
    - The robot decides which shelf to restock next.  
    - Metrics track robot tasks and shelf status in real-time.
    """
)
...
# -------------------------
def get_state():
    response = requests.get(f"{BACKEND_URL}/state")
...
st.write(
    "Click the button below to let the robot decide which shelf to restock next."
)
if st.button("Ask Robot to Restock"):
    response = requests.post(f"{BACKEND_URL}/decide")
...
st.write("---")
st.write(
    "💡 **Tip:** Try simulating multiple empty shelves and see how the robot decides which one to restock first!"
)
